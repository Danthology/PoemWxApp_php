...
class PoemController extends Controller
{
    public function getpoem()//
	{
		...
		$questionSessId=md5($redis->openid.$time.$QuesNo);
	...
	public function cut($content)//将诗分割
	{
	...
	public function answer()
	{
		...
		$skey=Cookie::get('skey');
		$redis=json_decode(Redis::get($skey));
		$find=DB::table('wx_account')->select('num')->where('openid',$redis->openid)->first();
		$num=$find->num;
		$question=json_decode(Redis::get($questionSessId));
		if($answer==$question->answer)
		{
			$newnum=$num+10;
			DB::table('wx_account')->where('openid',$redis->openid)->update(['num'=>$newnum]);
			$dan["answerCode"]=1;
			$dan["msg"]="答案正确";
		}
		else
